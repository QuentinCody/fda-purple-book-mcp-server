 ...
 */
export function isStrictReadOnly(sql) {
 ...
 */
export function validateTableAccess(tables, deniedTables = DENIED_TABLES) {
 ...
 */
export function redactRow(row, redactedColumns = REDACTED_COLUMNS) {
    const result = {};
 ...
 */
export function ensureLimit(sql, maxRows = MAX_ROWS) {
    const stripped = stripComments(sql);
...
export const directQueryTools = [
    {
        ...
        handler: async (input, ctx) => {
            const { sql: querySql, params } = input;
            ...
            // 4. Execute
            try {
                const rows = executeSql(ctx.sql, limitedSql, params);
    ...
    },
    {
        ...
        handler: async (input, ctx) => {
            ...
            for (const q of queries) {
                ...
                try {
                    const rows = executeSql(ctx.sql, limitedSql, q.params);
                    const truncated = rows.length > MAX_ROWS;
                    const resultRows = truncated ? rows.slice(0, MAX_ROWS) : rows;
