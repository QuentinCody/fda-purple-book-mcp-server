...
export function buildKnownEndpointIndex(catalog, openApiSpec) {
 ...
 */
export function preflightUnknownEndpoint(method, requestPath, knownEndpoints) {
    if (knownEndpoints.length === 0)
        return undefined;
    const pathMatchesAnyMethod = knownEndpoints.some((endpoint) => pathMatches(requestPath, endpoint.path));
    // The path is a real endpoint path — never pre-block (method may be valid but
    // undocumented; let upstream decide).
    if (pathMatchesAnyMethod)
        return undefined;
    const reqSegment = firstSegment(requestPath);
    if (!reqSegment)
        return undefined;
    const hasSibling = knownEndpoints.some((endpoint) => firstSegment(endpoint.path) === reqSegment);
    if (!hasSibling)
        return undefined;
...
export function buildDriftHint(method, requestPath, status, knownEndpoints) {
    if (knownEndpoints.length === 0)
        return undefined;
    const normalizedMethod = method.toUpperCase();
    const exactMatches = knownEndpoints.filter((endpoint) => endpoint.method === normalizedMethod &&
...
//# sourceMappingURL=api-proxy-drift.js.map