 ...
 */
import { z } from "zod";
import { paginateAll, } from "../codemode/paginate";
import { shouldStage, stageToDoAndRespond } from "../staging/utils";
import { buildStageOptions, interpolatePath, isRecord, validatePath, } from "./api-proxy";
/** Create the hidden __paginate_proxy tool entry. */
export function createPaginateProxyTool(options) {
    ...
    return {
        ...
        handler: async (input, ctx) => {
            ...
            const rawParams = isRecord(input.params)
                ? input.params
                : {};
            const opts = (isRecord(input.opts) ? input.opts : {});
            try {
                ...
                if (doNamespace &&
                    ...
                    const staged = await stageToDoAndRespond(pag.items, doNamespace, stagingPrefix, undefined, undefined, stagingPrefix, ctx?.sessionId, buildStageOptions(ctx, workspaceNamespace, stagingPrefix, pag.total_available));
                    return {
                        __staged: true,
...
//# sourceMappingURL=paginate-proxy.js.map