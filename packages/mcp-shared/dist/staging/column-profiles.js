...
function profileColumn(tableName, col, rowCount, sql) {
    ...
    const distinctResult = sql
        .exec(`SELECT COUNT(*) as c FROM (SELECT DISTINCT ${colRef} FROM "${tableName}" WHERE ${colRef} IS NOT NULL LIMIT ${PROFILE_DISTINCT_CAP})`)
        .one();
    ...
    try {
        const peek = sql
            .exec(`SELECT ${colRef} as v FROM "${tableName}" WHERE ${colRef} IS NOT NULL LIMIT 1`)
            .one();
        peekValue = peek?.v;
    }
    catch {
    ...
    if (col.type !== "JSON") {
        try {
            const minMaxResult = sql
                .exec(`SELECT MIN(${colRef}) as min_val, MAX(${colRef}) as max_val FROM "${tableName}" WHERE ${colRef} IS NOT NULL`)
                .one();
            if (minMaxResult) {
    ...
    if (col.type !== "JSON") {
        try {
            const sampleRows = sql
                .exec(`SELECT DISTINCT ${colRef} as v FROM "${tableName}" WHERE ${colRef} IS NOT NULL LIMIT ${PROFILE_SAMPLE_COUNT}`)
                .toArray();
    ...
    if (rawDistinct <= PROFILE_TOP_VALUES_THRESHOLD && rawDistinct > 0) {
        try {
            const topRows = sql
                .exec(`SELECT ${colRef} as v, COUNT(*) as c FROM "${tableName}" WHERE ${colRef} IS NOT NULL GROUP BY ${colRef} ORDER BY c DESC LIMIT ${PROFILE_TOP_VALUES_COUNT}`)
                .toArray();
            if (topRows.length > 0) {
