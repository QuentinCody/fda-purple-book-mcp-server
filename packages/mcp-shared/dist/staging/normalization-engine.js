...
export class NormalizationEngine {
    ...
    process(data, sql) {
        ...
        if (discovery.entities.size > 0) {
            schemas = buildSchemas(discovery.entities, discovery.relationships, this.config);
        }
        else {
        ...
        return {
            ...
            error: insertResult.errors.length > 0
...
// DDL helpers
// ---------------------------------------------------------------------------
function createTable(tableName, schema, sql) {
    ...
    for (const colName of Object.keys(schema.columns)) {
        if (colName.endsWith("_id") &&
            ...
            try {
                sql.exec(`CREATE INDEX IF NOT EXISTS idx_${tableName}_${colName} ON ${tableName}(${colName})`);
            }
            catch {
