...
import { ensureIdColumn, getSQLiteType, hasScalarFields, resolveColumnTypes, sanitizeColumnName, } from "./normalizer";
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
/**
 ...
 */
export function buildSchemas(entities, relationships, config) {
    const schemas = {};
    if (entities.size === 0)
        return schemas;
 ...
 */
export function buildFallbackSchema(data) {
    const schemas = {};
...
// ---------------------------------------------------------------------------
function extractEntityFields(obj, columnTypes, config) {
    const rowData = {};
                ...
                if (hasScalarFields(value)) {
                    ...
                    for (const [subKey, subValue] of Object.entries(value)) {
                        if (!Array.isArray(subValue) && typeof subValue !== "object") {
                            const prefixedColumn = columnName + "_" + sanitizeColumnName(subKey, config);
                            addColumnType(columnTypes, prefixedColumn, getSQLiteType(subValue));
...
// ---------------------------------------------------------------------------
function createJunctionTableSchemas(schemas, relationships) {
 ...
 */
function getEntityIdType(entityType, schemas) {
...
// ---------------------------------------------------------------------------
function createSchemaFromPrimitiveOrArray(data, tableName) {
    const columnTypes = {};
    const sampleData = [];
...
// Helpers
// ---------------------------------------------------------------------------
function addColumnType(columnTypes, column, type) {
