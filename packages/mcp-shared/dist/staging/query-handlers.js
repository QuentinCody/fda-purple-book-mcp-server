 ...
 */
import { createCodeModeError, createCodeModeResponse, } from "../codemode/response";
import { getRequestScope } from "../registry/request-scope";
import { getSchemaFromDo, queryDataFromDo } from "./utils";
import { getWorkspaceSchemaFromDo, queryWorkspaceFromDo, } from "./workspace-staging";
...
function queryCompleteness(result, limit) {
...
async function workspaceQuery(route, args, toolPrefix) {
    try {
...
async function perServerQuery(doNamespace, args, toolPrefix) {
    try {
 ...
 */
export function createQueryDataHandler(doBindingName, toolPrefix, handlerOptions) {
    return async (args, env) => {
        const route = resolveWorkspaceRoute(args, handlerOptions);
        if (route)
            return workspaceQuery(route, args, toolPrefix);
...
async function workspaceSchema(route, args, toolPrefix) {
    try {
...
async function perServerSchema(doNamespace, dataAccessId, toolPrefix) {
    try {
...
async function listStagedDatasets(doNamespace, toolPrefix, scope) {
    ...
    try {
        ...
        const registryDo = doNamespace.get(doNamespace.idFromName(resolvedScope ? `${resolvedScope}:__registry__` : "__registry__"));
        const listResp = await registryDo.fetch(new Request(`${DO_FETCH_ORIGIN}/list?session_id=${encodeURIComponent(resolvedScope || "")}`));
        const listResult = await parseJsonResponse(listResp, {
            success: false,
        });
 ...
 */
export function createGetSchemaHandler(doBindingName, toolPrefix, handlerOptions) {
    return async (args, env, scope) => {
        const route = resolveWorkspaceRoute(args, handlerOptions);
        if (route)
            return workspaceSchema(route, args, toolPrefix);
        const doNamespace = env[doBindingName];
        if (!doNamespace) {
            return createCodeModeError("DATA_ACCESS_ERROR", `${doBindingName} environment not available`);
        }
