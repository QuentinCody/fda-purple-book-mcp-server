...
export function isReadOnlyDescribe(sql) {
...
export function assertReadOnlySql(sql) {
    ...
    if (isReadOnlyDescribe(sanitizedSql))
        return sanitizedSql;
    ...
    if (compoundTerms > 450) {
        throw new Error(`Query has ${compoundTerms} compound-SELECT terms (UNION/INTERSECT/EXCEPT) — SQLite caps these near 500. ` +
...
export function applyDefaultLimit(sql, limit) {
    ...
    if (trimmed.toLowerCase().includes("limit"))
        return trimmed;
    return `${trimmed} LIMIT ${limit}`;
}
/** Strip a trailing `LIMIT n` so a query can be wrapped in `COUNT(*)`. */
export function stripTrailingLimit(sql) {
