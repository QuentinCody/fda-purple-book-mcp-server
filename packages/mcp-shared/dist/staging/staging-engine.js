 ...
 */
export function stageData(data, sql, context, hints, config) {
 ...
 */
function detectTier(data, config) {
 ...
 * mask a rows match.
 */
function nestedItemCandidates(value) {
    ...
    if (Array.isArray(wrapper.edges) && wrapper.edges.length > 0) {
        const firstNode = wrapper.edges[0]
            ?.node;
        if (firstNode)
 ...
 */
export function hasNestedEntities(obj, config) {
...
function runTier1(data, sql, hints) {
    ...
    for (const arr of arrays) {
        const tableName = schemaHints?.tableName ??
...
function storeFallbackPayload(data, sql) {
    sql.exec(`CREATE TABLE IF NOT EXISTS payloads (
...
//# sourceMappingURL=staging-engine.js.map