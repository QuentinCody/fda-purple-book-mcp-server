...
import { computeColumnProfiles, detectArrays, inferSchema, materializeSchema, } from "./schema-inference";
import { buildColumnDescriptor, buildColumnMeta, buildProfileByTable, buildRelationshipJoins, normalizeProvenance, } from "./schema-response";
...
export class RestStagingDO extends DurableObject {
    ...
    schemaValidatorInitFailed = false;
    constructor(ctx, env) {
        super(ctx, env);
    ...
    getSchemaValidator() {
        ...
        if (this.schemaValidatorInitFailed)
            return null;
        try {
            const row = this.sql
     ...
     */
    migrateMetadata() {
        this.sql.exec(`CREATE TABLE IF NOT EXISTS _do_migrations (
        ...
        const version = row.v;
        if (version < 1) {
            this.sql.exec(`CREATE TABLE IF NOT EXISTS _staging_metadata (
            ...
            this.sql.exec(`CREATE TABLE IF NOT EXISTS _inferred_schema (
					id INTEGER PRIMARY KEY,
					schema_json TEXT
				)`);
            this.sql.exec(`CREATE TABLE IF NOT EXISTS _column_profiles (
					id INTEGER PRIMARY KEY,
					profiles_json TEXT
				)`);
            this.sql.exec(`CREATE TABLE IF NOT EXISTS _session_registry (
					id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            this.sql.exec(`CREATE INDEX IF NOT EXISTS idx_session_registry_session_time
    ...
    storeProvenance(context) {
        if (context) {
            this.sql.exec(`INSERT INTO _staging_metadata (tool_name, server_name, args_json, api_url) VALUES (?, ?, ?, ?)`, context.toolName ?? null, context.serverName ?? null, context.args ? JSON.stringify(context.args) : null, context.apiUrl ?? null);
        }
    }
    /**
    ...
    updateProvenanceRowCounts(inputRows, storedRows, failedRows, warnings) {
        try {
            this.sql.exec(`UPDATE _staging_metadata SET input_rows = ?, stored_rows = ?, failed_rows = ?, warnings_json = ? WHERE id = (SELECT MAX(id) FROM _staging_metadata)`, inputRows, storedRows, failedRows, warnings.length > 0 ? JSON.stringify(warnings) : null);
        }
        catch {
    ...
    persistInferredSchema(schema) {
        try {
            ...
            for (const t of schema.tables)
                byName.set(t.name, t);
            const merged = { tables: Array.from(byName.values()) };
            this.sql.exec(`INSERT OR REPLACE INTO _inferred_schema (id, schema_json) VALUES (1, ?)`, JSON.stringify(merged));
     ...
     */
    persistColumnProfiles(schema) {
        try {
            const profiles = computeColumnProfiles(schema, this.sql);
            this.sql.exec(`INSERT OR REPLACE INTO _column_profiles (id, profiles_json) VALUES (1, ?)`, JSON.stringify(profiles));
        }
        catch {
    ...
    async handleProcess(request) {
        ...
        if (this.useConsolidatedEngine()) {
            ...
            const result = this.ctx.storage.transactionSync(() => stageData(data, this.sql, context, stagingHints, domainConfig));
            return this.jsonResponse({
        ...
        if (arrays.length > 0 && arrays.some((a) => a.rows.length > 0)) {
            try {
                ...
                const result = this.ctx.storage.transactionSync(() => materializeSchema(schema, rowsMap, this.sql));
        ...
        this.sql.exec(`CREATE TABLE IF NOT EXISTS payloads (
				id INTEGER PRIMARY KEY AUTOINCREMENT,
				root_json TEXT,
				created_at TEXT DEFAULT CURRENT_TIMESTAMP
			)`);
        ...
        const count = this.sql.exec(`SELECT COUNT(*) as c FROM payloads`).one()?.c ?? 0;
        return this.jsonResponse({
            success: true,
            table_count: 1,
            total_rows: count,
    ...
    async handleQuery(request) {
        ...
        if (body.count_total) {
            try {
                ...
                totalMatching = Number(countResult?.c ?? results.length);
    ...
    async handleQueryEnhanced(request) {
        ...
        const body = (rawEnhanced !== null && typeof rawEnhanced === "object"
            ? rawEnhanced
            : { sql: "" });
        // Pre-execution schema validation — catches column/table typos with suggestions
        const validationError = this.validateSql(body.sql);
        ...
        for (const row of rows) {
            ...
            for (const [k, v] of Object.entries(row)) {
                if (typeof v === "string" && this.chunking.isContentReference(v)) {
                    ...
                    const content = await this.chunking.retrieveChunkedContent(id, this.sql);
                    try {
        ...
        if (body.count_total) {
            try {
                const countSql = `SELECT COUNT(*) as c FROM (${stripLimit(body.sql)})`;
                const countResult = this.sql.exec(countSql).one();
                totalMatching = Number(countResult?.c ?? enhanced.length);
    ...
    async handleSchema() {
        ...
        try {
            ...
            if (profileResults.length > 0) {
                ...
                if (profileRow?.profiles_json) {
                    columnProfiles = JSON.parse(profileRow.profiles_json);
                }
            }
        }
        catch {
        ...
        const tableResults = this.sql
            .exec(`SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE '_staging_%' AND name NOT IN ('_inferred_schema', '_column_profiles')`)
            .toArray();
        for (const table of tableResults) {
            ...
            tables[tableName] = {
                row_count: rowCount,
                columns: columnResults.map((col) => buildColumnDescriptor(col, tableName, columnMeta, profileByTable)),
            };
        }
    ...
    async handleRegister(request) {
        ...
        this.sql.exec(`DELETE FROM _session_registry WHERE created_at < datetime('now', '-24 hours')`);
        this.sql.exec(`INSERT INTO _session_registry (session_id, data_access_id, tool_name, tables_json, total_rows, tool_prefix) VALUES (?, ?, ?, ?, ?, ?)`, body.session_id, body.data_access_id, body.tool_name ?? null, body.tables ? JSON.stringify(body.tables) : null, body.total_rows ?? null, body.tool_prefix ?? null);
        return this.jsonResponse({ success: true });
    }
    /**
        ...
        try {
            ...
            switch (action) {
                ...
                case "glob":
                    ...
                    break;
                default:
                    return this.jsonResponse({ success: false, error: `Unknown fs action: ${action}` }, 404);
            }
