 ...
 */
export function insertData(data, schemas, sql, config) {
    const state = {
...
// ---------------------------------------------------------------------------
function insertAllEntities(obj, schemas, sql, state, path, config) {
    ...
    if (isEntity(obj, config)) {
        ...
        if (schemas[entityType]) {
            const entityId = insertEntityRecord(obj, entityType, schemas[entityType], sql, state, config);
            // Track relationships for junction tables
            if (entityId !== null) {
                trackEntityRelationships(record, entityType, entityId, schemas, state, config);
            }
        }
    }
}
...
// ---------------------------------------------------------------------------
function insertEntityRecord(entity, tableName, schema, sql, state, config) {
...
function isAutoIncrementPk(columnName, schema) {
...
function resolveJsonColumn(columnName, record, config) {
...
function resolveDirectColumn(columnName, record, config) {
...
function resolveForeignKey(columnName, record, state, config) {
...
function resolveNestedField(columnName, record, config) {
    ...
    for (let splitPoint = 1; splitPoint < parts.length; splitPoint++) {
        ...
        if (originalKey &&
            ...
            const nestedObj = record[originalKey];
...
function resolveColumn(columnName, record, state, config) {
    return (resolveJsonColumn(columnName, record, config) ??
...
export function mapEntityToSchema(obj, schema, state, config) {
    const rowData = {};
    const record = obj;
    if (!obj || typeof obj !== "object") {
    ...
    for (const columnName of Object.keys(schema.columns)) {
        if (isAutoIncrementPk(columnName, schema))
            continue;
        const resolution = resolveColumn(columnName, record, state, config);
        if (resolution === "skip-column" || resolution === null)
            continue;
...
// Track relationships for junction tables
// ---------------------------------------------------------------------------
export function trackEntityRelationships(entity, entityType, entityId, schemas, state, config) {
    for (const [key, value] of Object.entries(entity)) {
        ...
        if (!schemas[junctionName])
            continue;
        const pairs = state.relationshipData.get(junctionName) ?? new Set();
...
// ---------------------------------------------------------------------------
function insertJunctionRecords(schemas, sql, state) {
    for (const [junctionName, pairs] of state.relationshipData.entries()) {
        ...
        for (const pairKey of pairs) {
            ...
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                state.errors.push(`INSERT into junction ${junctionName} failed: ${msg}`);
            }
        }
    }
}
...
// ---------------------------------------------------------------------------
function insertSimpleRow(obj, tableName, schema, sql, state, config) {
...
function mapObjectToSimpleSchema(obj, schema, config) {
    const rowData = {};
