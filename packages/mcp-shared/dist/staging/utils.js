...
export { createGetSchemaHandler, createQueryDataHandler, } from "./query-handlers";
...
export async function stageToDoAndRespond(data, doNamespace, prefix, schemaHints, provenance, toolPrefix, scope, options) {
    ...
    const processResult = await parseJsonResponse(processResp, {
        success: false,
    ...
    const schemaResult = await parseJsonResponse(schemaResp, {
        success: false,
    });
    ...
    const failedRows = typeof warnings.rows_skipped === "number"
        ? warnings.rows_skipped
        : undefined;
    const dataLossWarning = typeof warnings.data_loss_warning === "string"
