 ...
 */
export function storeWithVirtualColumns(rows, tableName, sql) {
    const errors = [];
    if (rows.length === 0) {
    ...
    if (columnDefs.length === 0) {
        ...
        sql.exec(`CREATE TABLE IF NOT EXISTS "${tableName}" (
				id INTEGER PRIMARY KEY AUTOINCREMENT,
				_raw_json TEXT NOT NULL
			)`);
        for (const row of rows) {
            try {
                sql.exec(`INSERT INTO "${tableName}" (_raw_json) VALUES (?)`, JSON.stringify(row));
            }
            catch (err) {
                errors.push(`INSERT into ${tableName} failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    ...
    const generatedCols = columnDefs
        .map((col) => `"${col.name}" ${col.sqliteType} GENERATED ALWAYS AS (json_extract(_raw_json, '$.${col.jsonPath}')) STORED`)
        .join(",\n\t\t");
    sql.exec(`CREATE TABLE IF NOT EXISTS "${tableName}" (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			_raw_json TEXT NOT NULL,
    ...
    for (const col of columnDefs) {
        if (col.name === "id" || col.name.endsWith("_id")) {
            try {
                sql.exec(`CREATE INDEX IF NOT EXISTS "idx_${tableName}_${col.name}" ON "${tableName}"("${col.name}")`);
            }
            catch {
...
function discoverColumns(samples) {
...
//# sourceMappingURL=virtual-columns.js.map