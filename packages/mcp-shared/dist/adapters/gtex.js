...
export async function gtexGet(path, params = {}, opts = {}) {
    ...
    try {
        ...
        if (!resp.ok)
            throw new Error(`GTEx HTTP ${resp.status}: ${await resp.text().catch(() => "")}`);
 ...
 */
export async function gtexEqtlsByVariants(variantIds, opts = {}) {
    const params = {
