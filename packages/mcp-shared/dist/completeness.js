 ...
 */
export function detectOverMatch(envelope) {
    if (!envelope || typeof envelope !== "object" || Array.isArray(envelope))
        return undefined;
    const meta = pickMeta(envelope);
    if (!meta)
        return undefined;
    const total = asCount(meta.total);
    if (total === undefined || total < OVER_MATCH_TOTAL_THRESHOLD)
        return undefined;
    const sortingOff = meta.sorted_by_relevance === false || meta.sort_field === null;
    if (!sortingOff)
        return undefined;
    return {
        warning: "filter_may_not_have_applied",
        total,
        detail: `Upstream reports ${total} matching record(s) with sorting/relevance disabled — the text/boolean ` +
