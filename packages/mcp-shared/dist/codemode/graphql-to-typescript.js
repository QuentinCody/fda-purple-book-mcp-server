...
function formatReturnType(field) {
 ...
 */
function scoreType(type, referencedTypes) {
    let score = 0;
    if (referencedTypes.has(type.name))
        score += 10;
    if (type.fields)
        score += Math.min(type.fields.length, 10);
    if (type.kind === "OBJECT")
        score += 2;
    if (type.kind === "ENUM")
        score += 1;
 ...
 */
export function introspectionToSummary(introspection, options) {
    ...
    // --- Key types ---
    const objectTypes = introspection.types.filter((t) => t.kind === "OBJECT" &&
...
//# sourceMappingURL=graphql-to-typescript.js.map