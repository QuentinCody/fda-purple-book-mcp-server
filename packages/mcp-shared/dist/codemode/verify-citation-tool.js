...
const DESCRIPTION = "Verify a citation's integrity anchor: recompute sha256(canonicalJson(data)) " +
 ...
 */
export function createVerifyCitationTool() {
    const schema = {
        expected_hash: z
            .string()
            .describe("The previously-issued result_hash (hex sha256) to verify against — " +
            "e.g. a Citation.result_hash."),
        data: z
            .unknown()
            .describe("The (claimed) underlying result data to re-hash. Any JSON value; " +
            "key order does not matter (canonicalized before hashing)."),
    };
    async function handle(input) {
        const expectedHash = input?.expected_hash;
        if (typeof expectedHash !== "string" || expectedHash.length === 0) {
            return createCodeModeError(ErrorCodes.INVALID_ARGUMENTS, "expected_hash is required and must be a non-empty hex string");
        }
        const { verified, expected_hash, actual_hash } = await verifyResultHash(expectedHash, input.data);
    ...
    return {
        ...
        register(server) {
            ...
            const toolHandler = (input) => handle(input);
...
//# sourceMappingURL=verify-citation-tool.js.map