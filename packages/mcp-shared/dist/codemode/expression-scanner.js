...
export function readQuotedString(source, start) {
...
export function parseSpecLookupTokens(expr) {
    let pos = 0;
...
export function splitTopLevelExpressions(source) {
    const parts = [];
    let current = "";
    let parenDepth = 0;
    let bracketDepth = 0;
...
export function parseCallExpressionAt(expr, start = 0) {
    let pos = start;
    while (pos < expr.length && /\s/.test(expr[pos]))
        pos++;
...
export function stripOuterParens(expr) {
...
export function findTopLevelArrow(expr) {
    let parenDepth = 0;
    let bracketDepth = 0;
    let braceDepth = 0;
    let quote = null;
...
export function findTopLevelOperator(expr, operators) {
    let parenDepth = 0;
    let bracketDepth = 0;
    let braceDepth = 0;
    let quote = null;
...
export function findTopLevelChar(expr, target) {
    let parenDepth = 0;
    let bracketDepth = 0;
    let braceDepth = 0;
    let quote = null;
...
export function parseMemberAccess(expr) {
...
export function parseArrowParam(source) {
...
export function parseOptionalMemberAccess(expr, start) {
    let pos = start;
    let optional = false;
