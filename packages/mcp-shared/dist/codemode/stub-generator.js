 ...
 */
function placeholderFromParam(p) {
    ...
    if (egMatch)
        return JSON.stringify(egMatch[1]);
    // Type-based fallback
    switch (p.type) {
        case "number":
            return "1";
 ...
 */
function placeholderFromOpenApiParam(p) {
    if (p.example !== undefined)
        return JSON.stringify(p.example);
    if (p.schema?.example !== undefined)
        return JSON.stringify(p.schema?.example);
    if (p.schema?.default !== undefined)
        return JSON.stringify(p.schema?.default);
 ...
 */
export function generateEndpointStub(ep) {
    if (ep.example)
        return ep.example;
 ...
 */
export function generateOperationStub(op) {
    ...
    const shownOptional = optionalQuery
        .filter((p) => p.schema?.default !== undefined ||
        p.schema?.enum ||
        p.example !== undefined)
        .slice(0, 2);
    for (const p of shownOptional) {
...
function generateCatalogQuickRef(catalog, max, prefix) {
    ...
    const sorted = [...catalog.endpoints]
        ...
        if (!a.coveredByTool && b.coveredByTool)
            return -1;
        if (a.coveredByTool && !b.coveredByTool)
            return 1;
        // More required params = more useful to show
        const aReq = (a.pathParams?.length ?? 0) +
            (a.queryParams?.filter((p) => p.required)?.length ?? 0);
        const bReq = (b.pathParams?.length ?? 0) +
...
function generateOpenApiQuickRef(spec, max, prefix) {
 ...
 */
function formatOpenApiParamType(p) {
    if (p.schema?.enum && p.schema?.enum.length > 0) {
        const vals = p.schema?.enum.slice(0, 5).map((v) => JSON.stringify(v));
        if (p.schema?.enum.length > 5)
            vals.push("...");
        return vals.join(" | ");
    }
 ...
 */
function formatEndpointTypeHint(ep) {
    const method = ep.method === "GET" ? "api.get" : "api.post";
    const allParams = [
 ...
 */
function formatOperationTypeHint(op) {
    const method = op.method.toLowerCase() === "get" ? "api.get" : "api.post";
    const params = op.parameters || [];
    const allParams = [
        ...params
