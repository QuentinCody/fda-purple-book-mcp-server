...
class SelectionParser {
    ...
    parseField() {
        ...
        const field = { name, args: [], hasArgs: false };
        if (this.s[this.i] === "(") {
            field.hasArgs = true;
    ...
    skipValue() {
        ...
        const c = this.s[this.i];
        if (c === undefined)
            throw new UnparseableError("missing value");
        if (c === '"') {
            this.skipString();
            return;
        }
    ...
    skipString() {
        if (this.s.startsWith('"""', this.i)) {
            ...
            if (end < 0)
                throw new UnparseableError("unterminated block string");
            this.i = end + 3;
            return;
        }
    ...
    readName() {
        ...
        while (this.i < this.s.length && /[A-Za-z0-9_]/.test(this.s[this.i]))
            this.i++;
        if (this.i === start)
            throw new UnparseableError("expected name");
        return this.s.slice(start, this.i);
    }
    expect(ch) {
        this.skipTrivia();
        if (this.s[this.i] !== ch)
            throw new UnparseableError(`expected ${ch}`);
        this.i++;
    }
...
export function isMutationOperation(query) {
    ...
    return (query.startsWith("mutation", i) && !/[A-Za-z0-9_]/.test(query[i + 8] ?? ""));
}
...
export function validateGraphqlQuery(query, introspection) {
    ...
    const walk = (selection, typeName) => {
        ...
        for (const sel of selection) {
            ...
            if (sel.selectionSet)
                walk(sel.selectionSet, namedType(def.type));
        }
    };
...
export function formatGqlValidationErrors(errors) {
    return errors.map((e) => e.message).join(" ");
}
//# sourceMappingURL=graphql-validate.js.map