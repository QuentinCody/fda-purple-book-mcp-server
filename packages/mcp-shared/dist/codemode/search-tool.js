 ...
 */
export function createOpenApiHelpers(specJson) {
    const HTTP_METHODS = [
        "get",
        "post",
        "put",
    ...
    let spec;
    try {
        spec = Object.freeze(JSON.parse(specJson));
    }
    catch (e) {
...
function createOpenApiSearchTool(prefix, spec) {
    ...
    return {
        ...
        schema: {
            code: z
                .string()
                .describe("JavaScript code to search the API spec. Use searchPaths(), listTags(), " +
            ...
            query: z
                .string()
                .optional()
                .describe("Legacy keyword search. Optional alternative to code. Use '*' or an empty string to browse operations."),
            category: z
                .string()
                .optional()
                .describe("Legacy category filter. Matches OpenAPI tags case-insensitively."),
            max_results: z
                .number()
                .optional()
                .describe("Maximum results to return for legacy keyword search (default 10, max 25)."),
        },
        register(server) {
...
function createCatalogSearchTool(prefix, catalog) {
    ...
    return {
        ...
        schema: {
            query: z
                .string()
                .describe("Search query — keywords matching endpoint paths, descriptions, parameters, or categories. Examples: 'gene expression', 'variant annotation', 'tissue'"),
            category: z
                .string()
                .optional()
                .describe("Filter to a specific category. Use query='*' with a category to list all endpoints in that category."),
            max_results: z
                .number()
                .optional()
