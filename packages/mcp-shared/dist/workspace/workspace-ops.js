 ...
 */
import { detectArrays, inferSchema, materializeSchema, } from "../staging/schema-inference";
...
export function ensureWorkspaceTables(sql) {
    sql.exec(`CREATE TABLE IF NOT EXISTS ${MANIFEST} (
 ...
 */
export function prefixSchema(schema, dataset) {
    return {
...
function newDataAccessId(dataset) {
...
function dropDatasetTables(sql, dataset) {
 ...
 */
export function stageDataset(sql, params) {
    ...
    const completeness = failedRows > 0
        ? { complete: false, failed_rows: failedRows }
        : { complete: true };
    sql.exec(`INSERT OR REPLACE INTO ${MANIFEST}
			(dataset, data_access_id, source_tool, tables_json, schema_json, row_count, completeness, created_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`, dataset, dataAccessId, params.sourceTool ?? null, JSON.stringify(tables), schema ? JSON.stringify(schema) : null, rowCount, JSON.stringify(completeness));
    return {
        dataset,
...
export function queryWorkspace(sql, params) {
...
function tableRowCount(sql, table) {
    const row = sql
...
export function workspaceSchema(sql, dataset) {
    ensureWorkspaceTables(sql);
    const manifestRows = (dataset
        ? sql.exec(`SELECT * FROM ${MANIFEST} WHERE dataset = ?`, dataset)
        : sql.exec(`SELECT * FROM ${MANIFEST} ORDER BY created_at, dataset`)).toArray();
    const datasets = manifestRows.map((r) => ({
...
export function clearWorkspace(sql) {
    ensureWorkspaceTables(sql);
