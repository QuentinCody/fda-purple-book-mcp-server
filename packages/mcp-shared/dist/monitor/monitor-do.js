...
export class MonitorDO extends DurableObject {
    ...
    constructor(ctx, env) {
        super(ctx, env);
        this.sql = ((strings, ...values) => this.ctx.storage.sql
    ...
    loadSub(id) {
        return (this
            .sql `SELECT * FROM subscription WHERE id = ${id} LIMIT 1`[0] ?? null);
    }
    firstActiveSub() {
        return (this
            .sql `SELECT * FROM subscription WHERE status = 'active' ORDER BY created_at LIMIT 1`[0] ?? null);
    }
    latestSnapshot(subId) {
        return (this
            .sql `SELECT * FROM monitor_snapshot WHERE subscription_id = ${subId} ORDER BY seq DESC LIMIT 1`[0] ?? null);
    }
    ...
    async changes(id) {
        const rows = this
            .sql `SELECT * FROM monitor_change WHERE subscription_id = ${id} ORDER BY id DESC LIMIT 200`;
