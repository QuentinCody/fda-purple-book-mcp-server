...
export function buildKeyFor(build) {
...
function assemblyName(build) {
    return build === "GRCh37" || build === "hg19" ? "GRCh37" : "GRCh38";
}
/** Parses a free-form variant string like `chr1-123-A-G`, `1:123_A/G`, etc. */
export function parseVariantString(value) {
    const raw = value.trim();
    if (!raw)
        throw new Error("Variant string is empty.");
    const parts = raw.split(SEP_RE).filter(Boolean);
    if (parts.length !== 4) {
        throw new Error("Invalid variant format. Expected chrom-pos-ref-alt with flexible separators.");
    }
...
export function buildVariantRecord(chrom, pos, ref, alt) {
    const record = { chr: chrom, pos, ref, alt };
...
export async function lookupRsid(rsid, build = "GRCh38", opts = {}) {
...
export async function lookupPosition(chrom, pos, build = "GRCh38", opts = {}) {
    const server = serverFor(build);
...
export async function resolveRsidBothBuilds(rsid, opts = {}) {
    const warnings = [];
    let g38 = null;
    let g37 = null;
    try {
        ...
        warnings.push(`GRCh38 lookup failed: ${err.message}`);
    }
    try {
        g37 = await lookupRsid(rsid, "GRCh37", opts);
    }
    catch (err) {
        warnings.push(`GRCh37 lookup failed: ${err.message}`);
    }
...
export async function resolveRsid(rsid, opts = {}) {
    if (!rsid.startsWith("rs"))
        throw new Error("rsid must start with 'rs'.");
    const both = await resolveRsidBothBuilds(rsid, opts);
    const ref = both.ref ?? null;
    const alt = both.alts[0] ?? null;
    const g37 = both.grch37.chr && both.grch37.pos != null
        ? buildVariantRecord(both.grch37.chr, both.grch37.pos, ref, alt)
        : null;
    const g38 = both.grch38.chr && both.grch38.pos != null
 ...
 */
export async function liftover(variant, fromBuild, toBuild, opts = {}) {
    ...
    if (!positionResult) {
        throw new VariantResolutionError("not_found", `No rsID found at ${variant.chr}:${variant.pos} on ${fromBuild}.`);
    }
    const targetCoord = await lookupRsid(positionResult.rsid, toBuild, opts);
    if (!targetCoord) {
        throw new VariantResolutionError("not_found", `rsID ${positionResult.rsid} not mappable to ${toBuild}.`);
    }
...
export async function resolveVariant(inputType, inputValue, opts = {}) {
    if (inputType === "rsid")
        return resolveRsid(inputValue, opts);
    const build = inputType === "grch37" ? "GRCh37" : "GRCh38";
    ...
    if (!positionResult) {
        throw new VariantResolutionError("not_found", `No rsID found at ${parsed.chr}:${parsed.pos} on ${build} via Ensembl overlap endpoint.`);
    }
    ...
    try {
        ...
        warnings.push(`Other-build lookup failed: ${err.message}`);
    }
    const sameBuild = buildVariantRecord(parsed.chr, parsed.pos, ref, alt);
    const otherBuildCoord = other
 ...
 */
export function formatForCohort(variant, _cohort) {
