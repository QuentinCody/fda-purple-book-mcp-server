 ...
 */
function matchSingleStarGlob(pattern, path) {
...
export class VirtualFS {
    ...
    ensureSchema() {
        ...
        this.#sql.exec(`INSERT OR IGNORE INTO _fs_entries (path, kind, size) VALUES ('/', 'directory', 0)`);
    ...
    ensureParentDirs(fullPath) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
            ...
            this.#sql.exec(`INSERT OR IGNORE INTO _fs_entries (path, kind, size) VALUES (?, 'directory', 0)`, current);
        }
    }
    // -----------------------------------------------------------------------
    // File operations
    ...
    writeFile(filePath, content) {
        ...
        this.#sql.exec(`INSERT INTO _fs_entries (path, kind, content, size, modified_at)
			 VALUES (?, 'file', ?, ?, datetime('now'))
			 ON CONFLICT(path) DO UPDATE SET
			   content = excluded.content,
			   size = excluded.size,
			   modified_at = datetime('now')`, normalized, content, size);
    ...
    appendFile(filePath, content) {
        ...
        const normalized = this.normalizePath(filePath);
        if (normalized === "/")
            throw new Error("Cannot append to root directory");
        const existing = this.#sql
            .exec(`SELECT kind, content, size FROM _fs_entries WHERE path = ?`, normalized)
            .toArray();
        if (existing.length > 0 && existing[0].kind === "directory") {
        ...
        if (existing.length === 0) {
            this.ensureParentDirs(normalized);
            this.#sql.exec(`INSERT INTO _fs_entries (path, kind, content, size) VALUES (?, 'file', ?, ?)`, normalized, newContent, size);
        }
        else {
            this.#sql.exec(`UPDATE _fs_entries SET content = ?, size = ?, modified_at = datetime('now') WHERE path = ?`, newContent, size, normalized);
        }
        return { path: normalized, size };
    }
    readFile(filePath) {
    ...
    mkdir(dirPath, options) {
        ...
        this.#sql.exec(`INSERT OR IGNORE INTO _fs_entries (path, kind, size) VALUES (?, 'directory', 0)`, normalized);
    }
    readdir(dirPath) {
        this.ensureSchema();
        const normalized = this.normalizePath(dirPath);
        ...
        const rows = this.#sql
            .exec(`SELECT path FROM _fs_entries WHERE path != ? AND path LIKE ? AND path NOT LIKE ?`, normalized, `${prefix}%`, `${prefix}%/%`)
            .toArray();
    ...
    stat(filePath) {
        ...
        const normalized = this.normalizePath(filePath);
        const rows = this.#sql
            .exec(`SELECT path, kind, size, created_at, modified_at FROM _fs_entries WHERE path = ?`, normalized)
            .toArray();
        if (rows.length === 0) {
    ...
    rm(filePath, options) {
        ...
        if (rows[0].kind === "directory") {
            ...
            const children = this.#sql
                .exec(`SELECT COUNT(*) as c FROM _fs_entries WHERE path LIKE ?`, `${prefix}%`)
                .toArray();
            ...
            if (childCount > 0) {
                this.#sql.exec(`DELETE FROM _fs_entries WHERE path LIKE ?`, `${prefix}%`);
            }
        }
    ...
    glob(pattern) {
        ...
        const rows = this.#sql
            .exec(`SELECT path FROM _fs_entries WHERE kind = 'file' AND path LIKE ? ESCAPE '\\'`, sqlPattern)
            .toArray();
